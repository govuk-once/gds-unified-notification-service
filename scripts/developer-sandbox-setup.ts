// This script is to be executed via pnpm run development:sandbox:setup
...
//     AS_DEVELOPER={their_email} pnpm run development:sandbox:setup
//
...
export const getConfig = () => {
  // Allow running `AS_ENVIRONMENT=dev pnpm run development:sandbox:setup` to repoint local TF to non sandbox setups
    ...
    for (const param of params) {
      ...
      if (getParameterResult && getParameterSandboxResult) {
        ...
        if (getParameterSandboxResult.Parameter?.Value == undefined) {
          console.log(
            `Parameter has not been initialized yet, please run pnpm run development:sandbox:release to deploy your environment first then come back to this flow.`
          );
          continue;
        }
        ...
        console.error(
          `Failed to fetch params from dev & sandbox environments - make sure to perform initial release of your sandbox (pnpm run development:sandbox:release)`
        );
      }
    }
  }
