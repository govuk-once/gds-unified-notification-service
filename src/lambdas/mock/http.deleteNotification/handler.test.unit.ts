...
import { MockDeleteNotification } from '@project/lambdas/mock/http.deleteNotification/handler';
...
describe('mockDeleteNotification Handler', () => {
  let instance: MockDeleteNotification;
  let handler: ReturnType<typeof MockDeleteNotification.prototype.handler>;
  ...
  const mockContext = {
    functionName: 'mockDeleteNotification',
    awsRequestId: '12345',
  ...
  beforeEach(() => {
    ...
    instance = new MockDeleteNotification(serviceMocks.configurationServiceMock, observabilityMocks);
    handler = instance.handler();
  });

  it('should have the correct operationId', () => {
    // Assert
    expect(instance.operationId).toBe('mockDeleteNotification');
  });

  it('should return 204 when a known notification ID is provided', async () => {
    // Arrange
