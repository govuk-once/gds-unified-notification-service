...
export const providerMetricDimensions = (config: EnvVars, group: string, provider: string): Record<string, string> => ({
...
export class UNSIntegrationAlarmsConstruct extends UNSAlarmsConstruct {
  ...
  constructor(scope: Construct, config: EnvVars, props: UNSIntegrationAlarmsProps) {
    ...
    for (const { name, provider, direction } of providers) {
      ...
      const providerMetric = (metricName: string): Metric =>
        new Metric({ namespace, metricName, dimensionsMap, statistic: Stats.SUM, period: AlarmPeriod.ONE_MINUTE });
