import { Alarm, Metric, Stats } from 'aws-cdk-lib/aws-cloudwatch';
...
export class UNSApiGatewayAlarmsConstruct extends UNSAlarmsConstruct {
  ...
  constructor(scope: Construct, config: EnvVars, props: UNSApiGatewayAlarmsProps) {
    ...
    const { restApiName, group } = props;
    const apiMetric = (metricName: string): Metric =>
      new Metric({
    ...
    const requests = apiMetric('Count');
